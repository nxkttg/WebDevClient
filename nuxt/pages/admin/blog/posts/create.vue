    ...
    <div class="mx-auto max-w-4xl px-6 py-10">
      ...
      <div class="mb-6">
        ...
        <p class="mt-3 text-base text-slate-500">
          Заповни форму, щоб додати нову статтю блогу.
        </p>
      </div>
...
const getCategories = async () => {
  try {
    ...
    if (response.data.length > 0) {
      ...
      state.category_id = String(response.data[0].id);
    }
  } catch (error) {
...
const onSubmit = async () => {
  ...
  try {
    await $fetch(`${config.public.apiBase}/admin/blog/posts`, {
      method: 'POST',
      body: {
        title: state.title,
    ...
    errorMessage.value = 'Не вдалося створити пост. Перевір дані у формі.';
  } finally {
    loading.value = false;
  }
};
