    ...
    <div class="mx-auto max-w-3xl px-6 py-10">
      ...
      <div class="mb-6">
        ...
        <p class="mt-3 text-base text-slate-500">
          Зміни назву, slug, батьківську категорію або опис категорії.
        </p>
      </div>
      ...
      <UCard class="shadow-sm">
        ...
        >
          ...
          <UFormGroup label="Батьківська категорія" name="parent_id" required>
            <select
          ...
          <UFormGroup label="Опис" name="description" hint="Необовʼязково">
            <UTextarea
...
const schema = z.object({
  ...
  parent_id: z.string().min(1, 'Оберіть батьківську категорію'),
...
const getCategory = async () => {
  ...
  try {
    const response = await $fetch<CategoryResponse>(
      `${config.public.apiBase}/admin/blog/categories/${categoryId}`,
    );

    state.title = response.data.title;
    state.slug = response.data.slug ?? '';
    state.parent_id = String(response.data.parent_id);
    state.description = response.data.description ?? '';
  } catch (error) {
    console.error('Помилка завантаження категорії:', error);
    errorMessage.value = 'Не вдалося завантажити категорію.';
  } finally {
